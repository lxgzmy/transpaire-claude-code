...
| `regress_hia.py` | **Passing** | Synthetic-value regression for `fill_hia.py` against BOTH staged HIA conversions (NSW + QLD): every anchor must resolve, the filled XML must parse, every synthetic value must appear in the filled text, and the $25.00 LD prefill must be replaced when the job sources a different figure. No client data. Skips with a notice on a checkout without the staged templates. Run after any `fill_hia.py` change — and after a new approved blank lands, point-check it with `fill_hia.py --check` before the first production fill. |
...
| `fill_hia.py` | **EXPERIMENTAL — commissioning tests only (CD-5.2b)** | HIA build-contract Word template + job JSON → filled `.docx`, label-anchored like the other fillers (`--check` first; a missing anchor stops it). `--region NSW` (default) or `QLD` picks the edition's anchor set: cover, Schedule 1 owners, The Land (NSW: DP NO / SUBURB POSTCODE; QLD: SP/RP and separate SUBURB/STATE/POSTCODE), and late-completion damages (REPLACES the template's $25.00 prefill when the job's sourced LD differs — NSW "per working day", QLD "per day"). Anchor matching is whitespace-normalised (the PDF reflow doubles spaces inside headings). NEVER fills date/price/GST/deposit/interest/margin/progress payments (CD-5.4). Output goes to template-testing only — the only existing blanks are the UNAPPROVED repaired conversions; regression-verification against the MCR-approved blank comes before the driver ever calls it. Commissioning tests: 26045 lot 143 (NSW, 16/16 anchors, 5-region diff), 25163 lot 13 (QLD, 14 anchors, 3-region diff, prefills verified against the executed contract). |
...
- **Build-contract figures and legal statements** (CD-5.4). The fill itself is
  ...
  date, prices, GST, deposit, progress payments (FROM DATABUILD), guarantors,
  special conditions and the resident-owner declaration are never filled — the
  data sheet lists them for a person. In PRODUCTION with no MCR-approved blank
  the whole contract stays a data sheet.
