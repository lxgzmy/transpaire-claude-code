...
| `fill_hia.py` | **EXPERIMENTAL — commissioning tests only (CD-5.2b)** | NSW/HIA build-contract Word template + job JSON → filled `.docx`, label-anchored like the other fillers (`--check` first; a missing anchor stops it). Fills the cover, Schedule 1 owners, The Land, and liquidated damages (REPLACES the template's $25.00 prefill when the job's sourced LD differs, flagged). NEVER fills date/price/GST/deposit/interest/margin/progress payments (CD-5.4). Output goes to template-testing only — the only existing blanks are the UNAPPROVED repaired conversions; regression-verification against the MCR-approved blank comes before the driver ever calls it. First test: 26045 lot 143, 16/16 anchors, 5-region diff. |
