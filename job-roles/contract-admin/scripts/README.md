...
| `regress_hia.py` | **Passing** | Synthetic-value regression for `fill_hia.py` against BOTH staged HIA templates (NSW team build + QLD conversion): every anchor must resolve, the filled XML must parse, every synthetic value must appear in the filled text, and every replaced prefill must be gone (the $25.00 LD figure; on NSW also the `$000,000.00`-style price placeholders when figures are supplied). Covers the legacy `land_suburb_pc` split too. No client data. Skips with a notice on a checkout without the staged templates. Run after any `fill_hia.py` change — and after a new approved blank lands, point-check it with `fill_hia.py --check` before the first production fill. |
