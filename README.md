...
|---|---|---|
| Contract Administration | Contract-documents workflow built and verified against real jobs: inclusions + prelim + HIA build contract filled and saved with per-document routing (CD-7.6/7.7, 8 Sep 2026: a document not yet in the job folder saves into it, one already there refreshes to template-testing; build contract per CD-5.2b - approved blank, else the staged interim template under the real name). New-job intake retired 18 Aug 2026; variations not started | [job-roles/contract-admin/](job-roles/contract-admin/README.md) |
