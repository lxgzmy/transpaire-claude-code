...
|---|---|---|
| Contract Administration | Contract-documents workflow built and verified against real jobs: inclusions + prelim + HIA build contract filled and saved with per-document routing (CD-7.6/7.7, 8 Sep 2026: a document not yet in the job folder saves into it, one already there refreshes to template-testing; build contract per CD-5.2b - approved blank, else the staged interim template under the real name). OSC intake integrated 11 Sep 2026 (issue #34): client, job, details, activities, request email and contacts through the `osc-api` MCP, each write approved; DataBuild stays manual. Variations not started | [job-roles/contract-admin/](job-roles/contract-admin/README.md) |
